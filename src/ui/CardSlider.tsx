      ...
      className="space-y-2 flex-col md:w-[450px] w-[300px] h-auto m-4 ">
        <button onClick={() => handleClick(Products.id)}>
          <img
            ...
            className="md:h-[290px] md:w-[620px]  h-[150px] rounded"
          />
        </button>
        <button onClick={(event) =>  addToCart(Products, event)} className="truncate lg:pl-[20rem]">
        <div style={{ backgroundColor: 'white', borderRadius: 4, padding: 4 }} className="absolute top-[20rem]">
        <img src='icons/buy.svg' className="text-[#2BA0AF]  rounded h-[1.3rem]" />
        </div>
      </button>
